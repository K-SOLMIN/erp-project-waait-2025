		...
		<c:if test="${not empty senderFavoriteMail }">
				<c:forEach var="senderMail" items="${senderFavoriteMail }">
					<li class="media" id="${senderMail.mailNo }" name="mailList">
						<div class="user-action">
							...
							<button class="icon-button" onclick="addFavorite(event)">
	                            ...
	                            >
									<svg class="bi" width="1.5em" height="1.5em" fill="currentColor">
		                            	<use xlink:href=<c:if test="${senderMail.senderStatus eq '즐겨찾기' }" >
		                                                	"${path }/resources/assets/static/images/bootstrap-icons.svg#star-fill"
		                                                </c:if>
														<c:if test="${senderMail.senderStatus != '즐겨찾기' }" >
		                                                  	"${path }/resources/assets/static/images/bootstrap-icons.svg#star"
					...
					<div class="media-body" onclick="goMailDetail(event)">
						<div class="user-details">
							<div class="mail-items">
								<span class="list-group-item-text text-truncate">${senderMail.senderMailAddress } 받은사람 : ${senderMail.senderName }</span>
							</div>
							<div class="mail-meta-item">
								<span class="float-right"> 
									<%-- <span class="mail-date">${mail.mailWriteDate }</span> --%>
